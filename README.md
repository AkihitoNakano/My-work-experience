...
また、新しい技術として現在では GO と PostgreSQL を勉強し次の開発で活用できるよう準備しています。
...
データベースは NoSQL の mongoDB を使用し続けてきましたが、国内では RDBMS を使用している企業が多く、その必要性から PostgreSQL の学習を始めました。SQL 文自体は以前からある程度学習しているため、PostgreSQL の基本を youtube や翻訳済み公式サイトで学習し、並行して Node.js のアプリケーション（家計簿アプリ）で連携できるよう実装の練習をしています。
...
※以下の解説ページにサンプルアカウントも載せておりご利用頂けますので閲覧頂けますと幸いです。
...
**UNREAL ENGINE** を使用したアマチュアのゲーム開発を個人で 5 年ほど経験しております。 ゲーム開発での経験を経てプログラミングに興味を持つきっかけとなりました。
